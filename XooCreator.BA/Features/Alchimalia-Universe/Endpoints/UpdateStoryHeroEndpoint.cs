...
[Endpoint]
public class UpdateStoryHeroEndpoint
{
    private readonly IStoryHeroService _service;
    private readonly IAuth0UserService _auth0;
    private readonly ILogger<UpdateStoryHeroEndpoint> _logger;

    public UpdateStoryHeroEndpoint(
        IStoryHeroService service,
        IAuth0UserService auth0,
        ILogger<UpdateStoryHeroEndpoint> logger)
    {
        _service = service;
        _auth0 = auth0;
        _logger = logger;
    ...
    public static async Task<Results<Ok<StoryHeroDto>, NotFound, BadRequest<string>, UnauthorizedHttpResult, ForbidHttpResult>> HandlePut(
        ...
        [FromServices] UpdateStoryHeroEndpoint ep,
        [FromBody] UpdateStoryHeroRequest req,
        CancellationToken ct)
    {
        ...
        {
            ep._logger.LogWarning("UpdateStoryHero forbidden: userId={UserId}", user?.Id);
            return TypedResults.Forbid();
        }

        try
        {
            var storyHero = await ep._service.UpdateAsync(user.Id, storyHeroId, req, ct);
