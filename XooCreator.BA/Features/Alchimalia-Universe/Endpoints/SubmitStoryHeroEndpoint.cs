...
[Endpoint]
public class SubmitStoryHeroEndpoint
{
    private readonly IStoryHeroService _service;
    private readonly IAuth0UserService _auth0;
    private readonly ILogger<SubmitStoryHeroEndpoint> _logger;

    public SubmitStoryHeroEndpoint(
        IStoryHeroService service,
        IAuth0UserService auth0,
        ILogger<SubmitStoryHeroEndpoint> logger)
    {
        _service = service;
        _auth0 = auth0;
        _logger = logger;
    ...
    public static async Task<Results<Ok<SubmitResponse>, NotFound, Conflict<string>, UnauthorizedHttpResult, ForbidHttpResult>> HandlePost(
        ...
        [FromServices] SubmitStoryHeroEndpoint ep,
        CancellationToken ct)
    {
        ...
        {
            ep._logger.LogWarning("SubmitStoryHero forbidden: userId={UserId}", user?.Id);
            return TypedResults.Forbid();
        }

        try
        {
            await ep._service.SubmitForReviewAsync(user.Id, storyHeroId, ct);
