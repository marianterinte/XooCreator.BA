...
[Endpoint]
public class PublishAnimalEndpoint
{
    private readonly IAnimalService _service;
    private readonly IAuth0UserService _auth0;
    private readonly ILogger<PublishAnimalEndpoint> _logger;

    public PublishAnimalEndpoint(
        IAnimalService service,
        IAuth0UserService auth0,
        ILogger<PublishAnimalEndpoint> logger)
    {
        _service = service;
        _auth0 = auth0;
        _logger = logger;
    ...
    [Route("/api/{locale}/alchimalia-universe/animals/{animalId}/publish")]
    [Authorize]
    public static async Task<Results<Ok<PublishResponse>, NotFound, Conflict<string>, UnauthorizedHttpResult, ForbidHttpResult>> HandlePost(
        ...
        [FromServices] PublishAnimalEndpoint ep,
        CancellationToken ct)
    {
        ...
        {
            ep._logger.LogWarning("PublishAnimal forbidden: userId={UserId}", user?.Id);
