...
[Endpoint]
public class CopyStoryEndpoint
{
    ...
    private readonly ILogger<CopyStoryEndpoint> _logger;

    public CopyStoryEndpoint(
        ...
        ILogger<CopyStoryEndpoint> logger)
    {
        _crafts = crafts;
    ...
    public record CopyStoryRequest
    {
        public bool CopyAssets { get; init; } = true;
    }

    public record CopyStoryResponse
    {
        public required string StoryId { get; init; }
        public required string OriginalStoryId { get; init; }
    }

    [Route("/api/stories/{storyId}/copy")]
    [Authorize]
    public static async Task<
        Results<
            Ok<CopyStoryResponse>,
            BadRequest<string>,
            NotFound,
        ...
        [FromBody] CopyStoryRequest request,
        [FromServices] CopyStoryEndpoint ep,
        CancellationToken ct)
    {
        var (user, authOutcome) = await ep.AuthorizeCreatorOrAdminAsync(ct);
        if (authOutcome == AuthorizationOutcome.Unauthorized) return TypedResults.Unauthorized();
        if (authOutcome == AuthorizationOutcome.Forbidden) return TypedResults.Forbid();
        ...
        if (!ep.HasOwnership(currentUser, craft, definition))
        {
        ...
        ep._logger.LogInformation(
            "Copy story completed: userId={UserId} source={SourceStoryId} newStoryId={NewStoryId} copyAssets={CopyAssets}",
            currentUser.Id,
            storyId,
            newStoryId,
            request.CopyAssets);
        ...
        return TypedResults.Ok(new CopyStoryResponse
        {
            StoryId = newStoryId,
    ...
    private async Task<(AlchimaliaUser? User, AuthorizationOutcome Outcome)> AuthorizeCreatorOrAdminAsync(CancellationToken ct)
    {
        ...
        {
            _logger.LogWarning("Copy story forbidden: userId={UserId}", user.Id);
    ...
    private bool HasOwnership(AlchimaliaUser user, StoryCraft? craft, StoryDefinition? definition)
    {
        ...
        if (craft != null && craft.OwnerUserId == user.Id)
        {
            return true;
        }

        if (definition != null && definition.CreatedBy == user.Id)
        {
            return true;
        }

        _logger.LogWarning("Copy story forbidden due to ownership mismatch: userId={UserId}", user.Id);
        return false;
    }
    ...
    private async Task TryCopyAssetsAsync(AlchimaliaUser currentUser, StoryCraft? craft, StoryDefinition? definition, string newStoryId, CancellationToken ct)
    {
        try
        {
            if (craft != null)
            {
                ...
                if (sourceEmail == null)
                {
                    _logger.LogWarning("Skipping asset copy for storyId={StoryId}: cannot resolve source email", craft.StoryId);
                    return;
                }
            ...
            if (definition != null)
            {
                var ownerId = definition.CreatedBy;
                var sourceEmail = ownerId.HasValue
                    ? await ResolveOwnerEmailAsync(ownerId.Value, currentUser, ct)
                    : currentUser.Email;

                if (string.IsNullOrWhiteSpace(sourceEmail))
                {
                    _logger.LogWarning("Skipping asset copy for published storyId={StoryId}: cannot resolve source email", definition.StoryId);
                    return;
                }
        ...
        {
            _logger.LogError(ex, "Asset copy failed for storyId={StoryId}", newStoryId);
        }
    }
