...
public class StoryAudioExportQueueWorker : BackgroundService
{
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            ...
            await GenerateAudioBytesAsync(testPage.Text, job.Locale, audioService, job.ApiKeyOverride, job.TtsModel, job.VoiceName, ct);
        }
        catch (Exception ex)
        {
            var errorMessage = ExtractErrorMessage(ex);
        ...
        var tasks = pages.Select(async page =>
        {
            ...
            try
            {
                var audioBytes = await GenerateAudioBytesAsync(page.Text, job.Locale, audioService, job.ApiKeyOverride, job.TtsModel, job.VoiceName, ct);
        ...
        CancellationToken ct)
    {
        ...
        if (chunks.Count == 1)
        {
            ...
            var (audioData, _) = await audioService.GenerateAudioAsync(chunks[0], locale, voiceNameOverride, null, apiKeyOverride, ttsModelOverride, ct);
            return audioData;
        }
        ...
        foreach (var chunk in chunks)
        {
            ...
            var (audioData, _) = await audioService.GenerateAudioAsync(chunk, locale, voiceNameOverride, styleInstructions, apiKeyOverride, ttsModelOverride, ct);
