   ...
   - După `dotnet publish`, înainte de restart App Service:
     ...
     - name: Run database scripts
       run: dotnet run --project XooCreator.DbScriptRunner \
             --connection "${{ secrets.AZURE_PG_CONNSTRING_DEV_EUROPA }}" \
...
- ✅ `V0007__seed_tree_model.sql` transpune în DB configurațiile Tree of Light (`TreeConfigurations`, `TreeRegions`, `TreeStoryNodes`, `TreeUnlockRules`) direct din `Data/SeedData/TreeOfLight/*.json`. ID-urile pentru noduri/reguli sunt deterministe (MD5 → int) iar insert-urile sunt idempotente (`ON CONFLICT`). Generatorul nu mai inserează `CREATE EXTENSION` (100% compatibil Azure):  
  ...
  ```
- ✅ `V0008__seed_lab_of_imagination.sql` mută toate datele folosite de Creature Builder din `Data/SeedData/LaboratoryOfImagination/i18n/*` în tabelele `BodyParts`, `Regions`, `Animals`, `AnimalPartSupports` + traducerile aferente (`BodyPartTranslations`, `AnimalTranslations`). Include toate localele disponibile (ro-ro, en-us, hu-hu), folosește GUID-uri determinate în PowerShell și `ON CONFLICT` pentru idempotentă. Generator:  
  ```powershell
  cd BA/XooCreator.BA/Database/Scripts/Generators
...
- ✅ Integrare GitHub Actions pentru dev/prod: workflow-ul `.github/workflows/db-scripts.yml` rulează `XooCreator.DbScriptRunner` (dry-run + execuție) pe fiecare push în `DEV` (folosește `AZURE_PG_CONNSTRING_DEV_EUROPA`) și pe fiecare push în `main` pentru PROD (`AZURE_PG_CONNSTRING_PROD_EUROPA`); poate fi declanșat manual și cu `workflow_dispatch target=dev|prod|both`.
- ✅ 2025-11-27: prima execuție completă pe DEV (workflow `Database Scripts Runner / db-dev`) a aplicat `V0001…V0008` fără erori după eliminarea dependenței `uuid-ossp` – confirmă că runner-ul funcționează cap-coadă în Azure.
- ✅ Workflow-ul de deploy PROD (`.github/workflows/deploy-xoocreator-ba.yml`, App Service `alchimalia-be`) rulează automat `XooCreator.DbScriptRunner` cu `AZURE_PG_CONNSTRING_PROD_EUROPA` (dry-run + execuție) înainte de publish, astfel încât aplicația și schema DB rămân sincronizate în același pipeline.
- ✅ `Program.cs` face doar connectivity-check (fără `MigrateAsync`), iar endpoint-ul `/debug/db-state` (env Development) citește acum statusul din `alchimalia_schema.schema_versions`.
- ✅ Rollback minim: `Database/Scripts/Rollbacks/R0001__drop_schema.sql` șterge și recreează schema + `schema_versions`, permițând rerularea tuturor scripturilor. Documentație în `Database/Scripts/Rollbacks/README.md` + runner suportă `dotnet run ... --rollback V0001`.
- 🔜 Următorii pași: completarea procesului de regenerare automatizată pentru seed-uri noi + adăugarea de rollback-uri specifice acolo unde e necesar (ex: revert parțial pe tabele de conținut).
