...
- ✅ Integrare GitHub Actions pentru dev/prod: workflow-ul `.github/workflows/db-scripts.yml` rulează `XooCreator.DbScriptRunner` (dry-run + execuție) pe fiecare push în `DEV` (folosește `AZURE_POSTGRES_CONNSTRING_DEV`) și pe fiecare push în `main` pentru PROD (`AZURE_POSTGRES_CONNSTRING_PROD`); poate fi declanșat manual și cu `workflow_dispatch target=dev|prod|both`.
...
- ✅ Workflow-ul de deploy PROD (`.github/workflows/deploy-xoocreator-ba.yml`, App Service `alchimalia-be`) rulează automat `XooCreator.DbScriptRunner` cu `AZURE_POSTGRES_CONNSTRING_PROD` (dry-run + execuție) înainte de publish, astfel încât aplicația și schema DB rămân sincronizate în același pipeline.
